...
const AdminMediaModalComponent: React.FC<AdminMediaModalProps> = ({
  ...
  const handleAddPhiloTag = (tagToAdd?: string, forceOriginal?: boolean) => {
  ...
  const handleAddStyleTag = (tagToAdd?: string, forceOriginal?: boolean) => {
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (val === 'Custom Category') {
                    ...
                    setMediaFormat('Custom Category');
                  } else {
                    setIsCustomCategory(false);
                    setMediaFormat(val as MediaFormat);
                  }
                }}
          ...
          <div className="p-4 rounded-xl bg-indigo-950/20 border border-indigo-900/40 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-mono text-xs font-bold transition flex items-center gap-1"
              >
                <Plus size={13} /> Add
              </button>
            </div>

            {/* Explicit Add Original option button when user types */}
            {newPhiloTag.trim() !== '' && (
              <div className="flex items-center gap-2 p-2 bg-slate-900/90 rounded-lg border border-indigo-500/40 text-xs font-mono animate-fade-in">
                ...
                <button
                  type="button"
                  onClick={() => handleAddPhiloTag(newPhiloTag.trim(), true)}
                  className="px-2.5 py-1 rounded bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-200 border border-indigo-500/50 font-bold text-xs transition flex items-center gap-1.5"
          ...
          <div className="p-4 rounded-xl bg-slate-950/60 border border-slate-800 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-mono text-xs font-bold transition flex items-center gap-1"
              >
                <Plus size={13} /> Add
              </button>
            </div>
            ...
            {newStyleTag.trim() !== '' && (
              <div className="flex items-center gap-2 p-2 bg-slate-900/90 rounded-lg border border-emerald-500/40 text-xs font-mono animate-fade-in">
                ...
                <button
                  type="button"
                  onClick={() => handleAddStyleTag(newStyleTag.trim(), true)}
                  className="px-2.5 py-1 rounded bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-200 border border-emerald-500/50 font-bold text-xs transition flex items-center gap-1.5"
          ...
          <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-6 space-y-5 text-slate-100">
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-800">
              ...
              <button
                ...
                className="px-4 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-400 text-slate-950 font-mono font-bold text-xs transition shadow"
              >
                Confirm Links & Save
              </button>
            </div>
